...
## 게임 링크 : https://rlatnwhd.github.io/Note-Survive/
...
## 🚀 게임 실행 방법
1. `index.html` 파일을 웹 브라우저로 열기 (혹은 링크로 이동)
2. 플레이어를 **WASD 또는 방향키**로 이동
3. **Game Start** 버튼 위로 이동 후 **SPACE** 키로 게임 시작
4. **Setting** 버튼으로 게임 사운드 조절
5. **Description** 버튼으로 상세 게임 설명 확인
...
### <img src="images/Attribute.png" width="20" alt="속성부여권"> 속성 부여권
- ⏰ **1분마다 1개 지급** (최대 4개)
- 🔑 무기 보유 시 **E키**로 사용
- ⚠️ **제약사항**:
  - 같은 속성 연속 부여 불가
...
### 🔊 사운드 시스템
- ✅ **배경 음악 (BGM)**: 페이지 간 이동 시에도 끊김 없이 지속 재생
- ✅ **전역 사운드 관리**: localStorage를 활용한 볼륨 및 재생 상태 유지
- ✅ **버튼 클릭음**: 메뉴 버튼 선택 시 재생
- ✅ **피격음 (Hit)**: 플레이어가 적에게 공격받을 때 재생
- ✅ **레벨업음**: 레벨업 카드 선택 화면 표시 시 재생

### ⚙️ 설정 시스템
- ✅ **음량 조절**: 메인 메뉴 우측 하단 설정 버튼
- ✅ **BGM 볼륨**: 0~100% 실시간 조절
- ✅ **효과음 볼륨**: 0~100% 실시간 조절
- ✅ **설정 저장**: 브라우저 로컬 스토리지에 자동 저장
